...
def get_neighbor_numbers(x: int, i: int) -> Tuple[int]:
    """
...
def get_MMLR_generators(
    ...
    """
    Формирование len(samples)*2*n генераторов соответствующих соседним числам из samples по всем возможным координатам

    n: длина ячейки для исследуемого МЛГ

    cipher_gen: функция-генератор, реализующая операцию шифрования для рассматриваемого алгоритма, которая должен принимать следующие аргументы: (block, key)

    samples: входной блок, для которого будет сформирован генератор из cipher_gen

    key: ключ, используемый всеми генераторами при шифровании некоторого значения из samples
    ...
    for i in range(length):
        ...
        for sample in samples:
            ...
            gen_first = MMLR(r, n, pp, mf, first)
            gen_second = MMLR(r, n, pp, mf, second)
...
def get_next_gens_states(n: int, samples_num: int, gens: List[List[Tuple[Generator]]]) -> List[List[Tuple[int, int]]]:
    """
    Получает выходные значения для переданных генераторов шифров для следующего раунда.
    Генераторы должны быть предварительно сформированы вызовом get_MMLR_generators
    ...
    gens: генераторы, полученные вызовом get_MMLR_generators
    """
...
def check_current_round(n: int, samples_num: int, states: List[List[Tuple[int]]]) -> bool:
    """
...
def get_MMLR_perfection_power(
    n: int,
    r: int,
    pp: List[int],
    mf: Callable[[int, int], int],
    samples_num: int,
    max_rounds: int,
    # key_n: int,
    #  key: int = 0
